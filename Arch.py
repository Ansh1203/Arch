...
def greet():
    ...
    conn.commit()
    conn.close()
    SR.speak("\nMyself Arch. How may I help you?")
    ...
    try:
        while(True):
            ...
            #asking for name
                ...
                SR.speak("My name is Arch and I'm here to serve you.")
            #How are you
...
if __name__=="__main__":
        ...
        root.resizable(0,0)
        root.title("Arch")
        root.iconbitmap('Arch.ico')
        root.configure(bg='white')
        scrollable_text=scrolledtext.ScrolledText(root,state='disabled',height=15,width=87,relief='sunken',bd=5,wrap=tk.WORD,bg='#090a0a',fg='#3ec20a')
        ...
        Speak_label=tk.Label(root,text="SPEAK:>",fg="#d1131c",font='"Times New Roman" 12 ',borderwidth=0,bg='white')
        Speak_label.place(x=250,y=300)
        """Setting up objects"""
        SR=Annex.SpeakRecog(scrollable_text)    #Speak and Recognition class instance
        Listen_Button=tk.Button(root,image=mic_img,borderwidth=0,activebackground='#db7414',bg='#db7414',command=Launching_thread)
